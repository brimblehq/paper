...
## Provider keys for Brimble Agent

[Brimble Agent](/agent/overview) uses a different secret: a model provider key under **Settings → API Keys → Bring your own key**. That key belongs to Anthropic, OpenAI, Google, xAI, or DeepSeek. It does not authenticate the Brimble HTTP API, the sandbox SDKs, or [Brimble MCP](/mcp/overview).

See [Keys and limits](/agent/keys-and-limits) for plans, session caps, and how to add a key.
...
- [Brimble Agent keys and limits](/agent/keys-and-limits), bring-your-own-key for the in-dashboard agent.
- [Brimble MCP](/mcp/overview), connect a client to manage Brimble with this key.

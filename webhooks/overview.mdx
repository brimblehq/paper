...
- **At-least-once.** A delivery may be retried if your endpoint fails or times out. Make your handler idempotent, keying on `data.id` + `event` works for most resources.
...
curl -X POST https://api.brimble.io/v1/webhooks/test \
  ...
  -d '{
    ...
    "payload": {
      "event": "deployment.success",
      "subscription_id": "65a1f4c2b8d9e7f3a2b1c0d4",
      "data": { "id": "test-deployment", "status": "ACTIVE" }
    }
  }'
```

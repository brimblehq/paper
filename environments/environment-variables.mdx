...
description: "Set, edit, and delete environment variables for a project."
---

Set, edit, and delete environment variables for a project. Variables are injected into both the builder and the running container.

## Prerequisites

- A project deployed on Brimble. If you haven't deployed yet, follow the [quickstart](../getting-started/quickstart).
...
<Frame caption="The Environment tab on a project.">
  <img src="/images/environments/add-env.jpg" alt="Environment tab on a project showing the variables table with name, value (with eye/eye-slash toggle), environment dropdown, secret/public indicator" />
</Frame>
...
<Frame caption="Bulk-importing env variables from a .env file.">
  <img src="/images/environments/add-many-env.jpg" alt="Bulk import modal with a textarea showing a few KEY=value lines and a comment line, plus the Import button" />
</Frame>
...
See [Reference shared and cross-project variables](/environments/env-references) for the full syntax, scoping rules, and chaining behavior.

## Build-time vs runtime

<Warning>
  **Adding or changing a variable doesn't affect a running deployment.** Variables are injected at build and start time, so the new value takes effect on the next deploy. Click **Redeploy** on the latest deployment after changing env vars, otherwise your container keeps running with the old values.
</Warning>

Variables are injected into the builder and into the running container. Anything you set is available during `install`, `build`, and `start` commands, and your code at runtime reads them like any other process environment variable.
...
- [Environments concept](/environments/overview), how environments and inheritance work.
- [Reference shared and cross-project variables](/environments/env-references), the `{{shared.X}}` and `{{@slug.X}}` syntax.
- [Files](/projects/files), mount a config or secret file into the container when a path is required.

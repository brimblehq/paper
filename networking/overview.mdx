...
Normal web services receive HTTP/1.1 from the gateway. HTTP/2 and gRPC are available for projects that have HTTP/2 enabled and serve HTTP plus gRPC on the same `PORT`.

See [HTTP, WebSockets, and gRPC](/networking/http) for the full behavior and service requirements.
...
By default, outbound IPs are unpublished and can change when a project moves hosts. Don't allowlist them.

If a bank, payment processor, or API requires a fixed outbound IP, enable [Static outbound IPs](/networking/static-outbound-ips) on a plan that includes the feature.

## Next steps

- [Custom domains](../domains/custom-domains), pointing your own domain at a project.
- [HTTP, WebSockets, and gRPC](/networking/http), how public protocols reach your service.

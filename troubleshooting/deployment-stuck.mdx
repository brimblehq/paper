...
This both helps you diagnose hangs and keeps the builder active.

### Cause 3: Out-of-memory thrashing

Your build is using too much memory and the builder is paging or thrashing instead of OOMing.
...
### Cause 1: App not listening on `$PORT` or `0.0.0.0`

Your code listens on a hardcoded port, or binds to `127.0.0.1` instead of all interfaces. The edge can't reach it either way.

**Fix:** Read `process.env.PORT` and bind to `0.0.0.0`. See [Application failed to respond](/troubleshooting/application-failed-to-respond) for per-framework start commands.
...
- [Build failures](/troubleshooting/build-failures), for deployments that fail with an error.
- [502 errors](/troubleshooting/502-errors), for deployments that go active but don't serve traffic.

...
Different from a [502](502-errors): a 502 means a deployment is live but returning errors. This means no live deployment exists yet — the new build never reached **Active**.

Different from a [stuck deployment](deployment-stuck): stuck deployments hang. This one explicitly failed.
...
**Fix:** read `process.env.PORT`. See the same patterns in [502 errors → Cause 1](502-errors#cause-1-service-isnt-listening-on-port).
...
If startup genuinely needs to be that long for legitimate reasons, see [stuck deployments → App takes too long to start](deployment-stuck#cause-3-app-takes-too-long-to-start) for options.
...
- [502 errors](502-errors), if a deployment goes Active but doesn't serve traffic.
- [Deployment is stuck](deployment-stuck), if a deployment never finishes (rather than failing).
- [Build failures](build-failures), if the build itself fails before the deploy phase.

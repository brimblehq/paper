...
<Frame caption="Live deployment logs stream by phase as the build runs.">
  <img src="/images/projects/build-logs-pipeline-section.jpg" alt="Live deployment logs drawer streaming output, with phase headers (Clone, Detect, Install, Build, Push, Deploy) collapsible, status chip showingIn pr" />
</Frame>
...
- Hacker: 1
- Developer and Team: 2

Developer and Team share the same concurrency model. If all your slots are busy, new deployments enter `pending` and start as soon as a slot opens. Cancel a queued deployment by clicking **Cancel** on the deployment in the dashboard.

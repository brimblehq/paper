...
The build emits a directory of files; Brimble serves those files from the edge.
...
**Auto-detected for:** Next.js (`next export`), Vite, Astro, Hugo, Jekyll, Eleventy, Gatsby, Docusaurus, MkDocs, plain HTML repos.

**No runtime container.** No `PORT`, no start command. Build the assets, Brimble caches and serves them.
...
**Replicas:** 1–10. 1 = single instance. 2+ = automatic failover with a load-balanced connection string.

...
To deploy your own MCP server (not from the marketplace), see [Deploy an MCP server](deploy-an-mcp-server). The flow is the same as a web service, the only Brimble-specific extra is the authentication header.
...
- [Deploy an MCP server](deploy-an-mcp-server), deploy your own custom MCP code.
- [Service types](service-types), what MCP servers run as on Brimble.

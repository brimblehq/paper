...
Under the hood this uses a [volume](../volumes/overview) of type `web`. Same storage model, managed from the project **Configuration** tab.
...
The mount path decides **what** lives on durable storage and **what** stays on the fast local disk of the container.

| Goal | Mount path pattern |
| --- | --- |
| Generic API or worker | `/data` (or any empty data dir your app writes to) |
| WordPress Docker image | `/var/www/html/wp-content` (uploads, plugins, themes) |
| n8n / similar tools | the path the image documents for its data dir |
| SQLite | the directory that holds the database file |

**Why this matters:** persistent disks are durable and host-portable, but they are not as fast as local disk for huge numbers of small reads and writes. If you mount the volume over the full app root (for example `/var/www/html` for WordPress, or `/app` for a Node service), every request may pull framework files through storage. That can show up as slow first loads, install wizards timing out, or the proxy reporting that the app is unavailable while PHP or your runtime is still working.

**Rule of thumb:** image = code; volume = user data and state.

<Tip>
  Deploying a Docker image with a volume? Prefer the image's documented data directory, not the image's install root. See [Volumes](../volumes/overview#what-belongs-on-a-volume) for more examples.
</Tip>

## How persistent disks are stored

Brimble persistent disks (for projects and for [sandboxes](../sandboxes/overview)) use **Brimble's distributed object storage**, not a local SSD on the host running your container. Practical consequences:
...
- **I/O is networked.** Expect durable cloud-storage latency, not bare-metal disk speeds. SQLite, uploads, and modest caches are fine. Write-heavy databases belong on a [managed database](deploy-a-database). Apps that load thousands of tiny files per request should keep those files on the image and only put mutable data on the disk.
...
**App is very slow or times out only with a disk attached.** The mount path may cover the whole application tree (for example `/var/www/html` or `/app`) so every request pays storage latency for core files. Remount only the data directory (for WordPress, `/var/www/html/wp-content`), redeploy, and keep the app itself on the image.
...
* [Files](/projects/files), small config and secret files mounted at a path (not writable app data).
* [Volumes](/volumes/overview), lifecycle, types, and what belongs on a volume.
* [Deploy a database](/projects/deploy-a-database), for state that needs scale-out, backups, and queryability.
* [Deploy from a Docker image](/projects/deploy-from-docker-image), when the image has its own data directory conventions.

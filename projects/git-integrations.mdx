...
You can also deploy from a pre-built Docker image instead; see [Deploy from a Docker image](deploy-from-docker-image). That path doesn't require a git provider connection.
...
When you push a commit to a project's tracked branch, Brimble's webhook fires and a build kicks off automatically. The build uses the latest commit on that branch. See [Builds](builds) for the pipeline.
...
- [Deploy from a Docker image](deploy-from-docker-image), the alternative source if you don't want git-based builds.
- [Builds](builds), what happens after Brimble pulls the commit.
- [Deployments](deployments), how a build becomes a running service.

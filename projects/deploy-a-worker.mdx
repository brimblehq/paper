...
description: "Run a long-lived background process, queue consumer, scheduler, message handler, without exposing an HTTP port."
---
...
<Frame caption="Creating a worker project, source and branch.">
  <img src="/images/projects/deploy-worker-website.jpg" alt="New-project flow with service type set to Worker and the Git source selected" />
</Frame>

<Frame caption="Creating a worker project, build configuration.">
  <img src="/images/projects/deploy-worker-website-bottom.jpg" alt="New-project flow showing the worker build commands and start command" />
</Frame>
...
Add these under **Environment** before deploying. See [Manage environment variables](/environments/environment-variables).
...
By default a worker runs as a single container. To run more than one in parallel, configure an [autoscaling group](../scaling/overview) on the project. Each container runs the same image with the same env vars; your code is responsible for distributing work safely across them, typically by consuming from a shared queue with a consumer-group pattern.
...
- [Deployments](/projects/deployments), how restarts and health checks work.
- [Deploy a database](/projects/deploy-a-database), provision the queue or DB the worker reads from.

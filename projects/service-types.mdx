...
<Warning>
  **Next.js (and any other framework with both SSR and static modes) must be explicitly configured for static output.** A vanilla Next.js repo defaults to SSR, the build emits a `.next/` directory that needs a Node server at runtime. A static-site deploy doesn't have a runtime, so the build runs, the upload succeeds, and the deployed site loads blank.

  To deploy Next.js as a static site, set `output: "export"` in your `next.config.js` (or `.mjs`/`.ts`):
  ...
  This makes Next.js emit a real `out/` directory of HTML/CSS/JS at build time. **You lose server-side rendering, API routes, middleware, on-demand revalidation, and image optimization** — anything that needs a server at runtime. If your app uses any of those, deploy as a [Web service](#web-service) instead (paid plans only).

  Same caveat applies to SvelteKit (use the static adapter), Nuxt (`nuxt generate`), Remix (static adapter), and Astro/Hugo/Eleventy/Jekyll — but these latter ones default to static, so they Just Work.
</Warning>

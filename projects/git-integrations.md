...
Brimble supports four git providers as deployment sources: **GitHub**, **GitLab**, **Bitbucket**, and **Hugging Face**. Each one connects to your account separately. Once connected, you can deploy any repo your account has access to.

You can also deploy from a pre-built Docker image instead; see [Deploy from a Docker image](deploy-from-docker-image.md). That path doesn't require a git provider connection.
...
| Hugging Face | OAuth | Yes | Limited | No | Auto-registered per repo on connect |

The auth model is the main difference. GitHub uses an App install at the org or user level, you authorize Brimble's GitHub App once and it covers every repo you grant it access to. GitLab, Bitbucket, and Hugging Face use OAuth: you authorize Brimble, Brimble holds a token, and uses it per-repo.

## Connect a provider

You connect providers from the new-project flow or from your account settings.

1. In the dashboard, click **New project**, then pick the source: GitHub, GitLab, Bitbucket, or Hugging Face.
...
4. **GitLab, Bitbucket, Hugging Face:** authorize Brimble through OAuth. Brimble gets read access to your repos.
...
When you push a commit to a project's tracked branch, Brimble's webhook fires and a build kicks off automatically. The build uses the latest commit on that branch. See [Builds](builds.md) for the pipeline.
...
- **Hugging Face:** Brimble subscribes to the repo's events through OAuth.
...
- **Hugging Face:** Pull request handling is limited.

PR previews are gated by plan, see [Plans and pricing](../billing/plans.md).

## Commit status

For GitHub, GitLab, and Bitbucket, Brimble updates the commit status on the provider when a deploy starts and when it finishes. You'll see green check or red X marks next to commits in the provider's UI. The status links back to the deployment in Brimble.

Hugging Face doesn't get commit status updates.

## Hugging Face specifics

The Hugging Face integration is narrower than the others. Brimble can read your HF repos, fetch branches and content, and trigger builds from pushes. It doesn't post commit statuses back, and PR handling is limited compared to the GitHub/GitLab/Bitbucket flows.

Use the Hugging Face integration when you specifically want to deploy from an HF repo. For general application deployments, prefer GitHub, GitLab, or Bitbucket.
...
- **GitLab, Bitbucket, Hugging Face:** revoke Brimble's OAuth token in the provider's authorized-apps settings. The next attempt to deploy from a repo behind that token fails until you reconnect.
...
All four providers support private repos. The auth flow grants Brimble exactly the access scope you authorize:
...
- Hugging Face OAuth: account-level access.
...
**PR opened but no preview deployment.** PR previews are plan-gated. Confirm your plan allows them; see [Plans and pricing](../billing/plans.md). Also confirm the PR targets the tracked branch.
...
- [Deploy from a Docker image](deploy-from-docker-image.md), the alternative source if you don't want git-based builds.
- [Builds](builds.md), what happens after Brimble pulls the commit.

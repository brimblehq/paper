...
```text
Container exits → restart on same host (up to 10 attempts, 20s delay)
   │
   │  still failing
   ▼
Reschedule onto another host (exponential backoff, no limit)
   │
   │  host stops responding entirely
   ▼
After 15 min unreachable → mark allocation lost
   │
   ├── Web/MCP/DB → start a replacement immediately, keep replacement
   └── Worker     → wait for original (no double-up); reschedule if host is confirmed dead
```

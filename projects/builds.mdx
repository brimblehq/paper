...
<Check>
  **Secrets stay encrypted at rest and out of image layers.** Vault holds plaintext only in memory, the runner reads them with a short-lived token scoped to your project, and BuildKit's `--secret` mount keeps them out of layer history. There's no path that writes plaintext secrets to disk on the build host or into the image you ship.
</Check>
...
**Web services, MCP servers, workers, and databases: orchestrate as a container.**
The runner hands the artifact off to the orchestration layer, which schedules the container in your project's region. Brimble uses Nomad for scheduling and Consul for service discovery and health checking. For each deployment, the orchestration layer:

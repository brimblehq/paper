...
For a static site, the build phase ends with the asset bundle being uploaded to Brimble's **globally distributed object storage** for static artifacts. There's no container, no Nomad scheduling, no health probe. As soon as the upload completes, Brimble's gateway is able to serve the new assets directly from object storage on the next request, and Cloudflare's cache for your project's hostnames is purged so users see the new version immediately. See [Static sites](service-types#static-site) for the full flow and [Edge caching](#edge-caching) below for what gets cached and where.

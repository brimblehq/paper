...
**Provisioning is asynchronous from the create request.** Creating a sandbox returns immediately with `status: "starting"`; the sandbox isn't usable until the container is up. The SDK's `createReady` / `getReady` helpers wait for the flip for you; in the dashboard, the status chip updates on its own.

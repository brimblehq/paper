...
**Provisioning is asynchronous from the create request.** `POST /sandboxes` returns immediately with `status: "starting"`; the sandbox isn't usable until the container is up. Poll the detail endpoint or watch the dashboard for the `ready` flip.
...
`POST /sandboxes` arrives with a template, region, optional specs, and optional persistence flags. Brimble validates the request synchronously:
...
Any check failing returns a clear error. Otherwise the response comes back in under a second with `status: "starting"` and the sandbox ID.

### 2. Provisioning

The provisioner takes over and:
...
The container's network mode depends on the sandbox config: by default, normal outbound network; if `blockOutbound: true` was passed, outbound traffic is denied by a CNI policy.
...
- **`exec` / `code`** run a command or snippet inside the container and stream stdout/stderr back. Each call is bounded by a timeout (default 30s, capped at 300s).

...
Once a sandbox is `ready`:

| Operation | Endpoint | What it does |
|---|---|---|
| Exec command | `POST /sandboxes/{id}/exec` | Runs a shell command; returns stdout, stderr, exit code, duration. |
| Run code | `POST /sandboxes/{id}/code` | Runs a Python or Node snippet you pass in the body. Same return shape as exec. |
| Write file | `PUT /sandboxes/{id}/files/{path}` | Streams the body to the file at `path` inside the sandbox. |
| Read file | `GET /sandboxes/{id}/files/{path}` | Streams the file at `path` back to you. |
| Pause | `POST /sandboxes/{id}/pause` | Scales the container to zero. Persistent volumes stay attached. |
| Resume | `POST /sandboxes/{id}/resume` | Starts a fresh container and reattaches the volume. |
| Snapshot | `POST /sandboxes/{id}/snapshots` | Captures the sandbox's filesystem as a restorable image. See [Snapshots](snapshots). |
| Destroy | `DELETE /sandboxes/{id}` | Terminal cleanup. |

The full API is documented under the [Sandbox API](/sandboxes) tab.

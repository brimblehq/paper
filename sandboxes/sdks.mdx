...
```python Python
# Dedicated stream method returns a requests.Response with iter_lines()
response = sandbox.exec_stream({"cmd": "pip install -r requirements.txt"})
...
for line in response.iter_lines(decode_unicode=True):
    ...
    if frame["type"] in ("stdout", "stderr"):
...
```go Go
// Dedicated stream method returns an io.ReadCloser of newline-delimited JSON
rc, err := handle.ExecStream(ctx, sandbox.ExecInput{
    Cmd: "pip install -r requirements.txt",
})
...
for scanner.Scan() {
    ...
    case "done":
        fmt.Println("exit:", frame["exit_code"])
    }
}
```
...
The Python and Go SDKs also expose dedicated `run_code_stream` / `RunCodeStream` for streaming the language-aware variant. In TypeScript, the same `stream: true` opt-in works on `runCode` as well, the return type narrows to a readable stream.
...
- **Streams.** TypeScript file download returns `ReadableStream<Uint8Array>`. Python returns a `requests.Response` (use `iter_content` / `iter_lines`). Go returns `io.ReadCloser`. The same shapes apply to streaming `exec` / `runCode`, see [Stream exec output](#stream-exec-output).
- **Streaming exec/code.** TypeScript narrows on `stream: true` (the call returns `ReadableStream<Uint8Array>`). Python and Go expose dedicated `exec_stream` / `ExecStream` and `run_code_stream` / `RunCodeStream` methods that return their language-native stream type.
- **Batch uploads.** All three SDKs ship `putFiles` / `put_files` / `PutFiles` on the handle (`putFiles` / `put_batch` / `PutBatch` on the lower-level `FilesResource`). The cap is 100 files per request.

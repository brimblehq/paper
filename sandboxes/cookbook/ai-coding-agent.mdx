...
4. **Outbound network is on by default.** The agent needs to reach the model API (`api.anthropic.com`, `api.openai.com`, etc.). For tighter control, use `egress: { mode: "restricted", allow: ["api.anthropic.com"] }` instead of full `open`, or pre-bake tools into a snapshot and run with `deny_all`.

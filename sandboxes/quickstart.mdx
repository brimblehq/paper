...
The recommended path is the **SDK**. It handles polling, retries, idempotency, and serialization for you, so the snippets below are the shortest working version of the full lifecycle. The dashboard and REST paths are linked at the bottom if you prefer to click or curl your way through.
...
`region: "auto"` lets the server pick the best region for your account. Pass a specific region ID to pin one.
...
If you can't use an SDK, every step above maps to a single HTTP call:
...
curl -X POST https://sandbox.brimble.io/sandboxes \
  ...
  -d '{ "template": "node-22", "region": "auto", "name": "my-first-sandbox" }'

# Exec
curl -X POST https://sandbox.brimble.io/sandboxes/$SANDBOX_ID/exec \
  -H "x-brimble-key: $BRIMBLE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{ "cmd": "echo hello && uname -a" }'
...
The REST endpoints are documented under the [Sandbox API](/sandboxes) tab.

...
description: "Brimble pricing has three pieces:"
---
...
<Warning>
  **Picking a size above your plan's included compute is metered, not blocked.** The sliders let you go all the way up to 8 vCPU / 16 GB on every plan. Brimble doesn't stop you, the gap above your plan's included CPU/memory is billed at $4/GB-month for each, prorated for the time you ran at that size. Bump the slider deliberately, and watch **Billing → Usage** for the live estimate.
</Warning>
...
<Warning>
  **Free-plan projects can quietly meter overage.** Because the smallest slider stop (0.5 vCPU / 0.5 GB) is double the Free-plan baseline (0.25 / 0.25), every Free-plan project provisioned at the slider's minimum is paying metered compute on the 0.25-unit gap. Watch **Billing → Usage** if you didn't expect a charge.
</Warning>
...
Each plan includes a monthly build-minute allowance. A build minute is wall-clock time inside the build runner, from clone start to push end. See [Builds](../projects/builds) for what counts and what doesn't.

When you exhaust the allowance:

* **Free plan:** new builds queue indefinitely until the cycle resets.
* **Paid plans:** overage bills at $0.002/min and rolls into the next invoice. You can also top up with credits, see [Build minutes](build-minutes).
...
When a charge fails, Brimble doesn't run its own retry schedule, **Stripe Smart Retries** drives the retry cadence. Stripe picks the retry times based on its own machine-learning signals, so the exact gap between attempts varies, but the user-visible escalation is:

1. **First two failed attempts.** Stripe retries the card on its own schedule. Brimble emits a `payment.failed` webhook and an in-app warning on each failed attempt. Builds and runtime keep working.
2. **Three or more failed attempts.** Builds are disabled for the workspace. Existing deployments keep serving traffic. You'll get an additional in-app message that builds were disabled because of repeated payment failures.
3. **Stripe exhausts its retry attempts.** The subscription moves to `unpaid` and **projects are suspended**. Brimble emits a `subscription.unpaid` webhook and shows a banner asking you to settle the open invoice. The full window (typically up to 4 weeks, exact length is whatever you have configured on the Stripe side) is governed by Stripe.
4. **Thirty days after going unpaid.** Brimble cancels the subscription with reason `stale_unpaid_30_days`. The workspace drops to the Free plan. Projects above the Free plan limits stay in place but you can't create new ones over the limit.

<Warning>
  **Projects are suspended once Stripe stops retrying.** Once your subscription is `unpaid`, all projects in the workspace stop serving traffic until you settle the open invoice. After 30 days unpaid, the subscription is canceled outright. Keep a working card on file.
</Warning>

To recover, open **Billing → Invoices**, click the open invoice, and pay it (or update the payment method first under **Billing → Payment methods**). As soon as the invoice settles, the subscription leaves `unpaid`, projects come back online, and builds re-enable.
...
<Warning>
  **Free-plan projects auto-pause after 30 minutes of inactivity.** A request after that wakes the project, but the first request takes a cold-start hit (a few seconds for most stacks). Team and paid-plan projects are never idled this way. If you're hosting something a user might hit at any time, put it on Hacker or Pro.
</Warning>
...
* [Build minutes](build-minutes), top-ups and the included allowance.
* [Persistent disk](../projects/persistent-disk), how disk pricing scales.
* [Project metrics](../observability/metrics), the live usage view per project.

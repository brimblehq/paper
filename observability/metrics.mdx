...
- **CPU usage**, percentage of allocated CPU in use, with the allocation in vCPU shown alongside (for example `7.2% - 2.0 vCPU`).
- **Memory usage**, percentage of allocated memory in use, with the allocation in GB (e.g. `1.8% / 1.5 GB`).
...
See [Logs](/observability/logs) for the full breakdown.

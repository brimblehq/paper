...
Add, edit, and delete DNS records for a domain managed by Brimble. This guide covers the dashboard UI; for the supported record types and field shapes, see [DNS record types](/domains/dns-records).
...
If you just want to forward mail to an existing inbox rather than run a full mail provider, use [Email forwarding](/domains/email-forwarding) instead, no manual `MX` records needed.
...
- [DNS record types](/domains/dns-records), every record type and its fields.
- [DNS troubleshooting](../troubleshooting/dns), when records aren't behaving.
- [Add a custom domain](/domains/custom-domains), point a Brimble-managed domain at a project.
- [Email forwarding](/domains/email-forwarding), route mail on this domain to an inbox you already use.

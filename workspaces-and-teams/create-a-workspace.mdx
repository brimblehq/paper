...
description: "A workspace holds projects shared with a team."
---

A **workspace** holds projects shared with a team. Workspaces are billed separately from your personal account and have their own seats, projects, and usage.
...
<Frame caption="Step 1: name the workspace and pick its URL slug.">
  <img src="/images/workspaces-and-teams/workspace-creation-1.jpeg" alt="Workspace creation step 1 with the workspace image upload, name field, and URL slug input prefixed by the brimble.app domain" />
</Frame>

## Step 2: Seats and options

This step sets the workspace's recurring Team charge.

| Setting | Description |
|---|---|
| **Team size** | Number of seats. You can start with any count of **1** or more. Each seat costs **$11/month**. |
...
The page shows a running total based on seats × **$11**.

A workspace with **3 seats** is **$33/month** for the Team subscription, before metered usage (compute, bandwidth overage, storage, and so on). See [How billing works](/billing/plans) and [Compute pricing](/billing/compute).

You can change seat count later under **Workspace settings → Billing**, but you can't go below the seats currently in use without removing members first.

Click **Continue**.

<Frame caption="Step 2: pick team size, with the running monthly total.">
  <img src="/images/workspaces-and-teams/workspace-creation-2.jpeg" alt="Workspace creation step 2 with team-size selector, optional startup promo code field, and the running monthly cost summary" />
</Frame>
...
The page shows a billing summary for seats. If you've added more invites than your team size, a warning appears: "You've invited N members but only have M seats, additional seats will be charged."
...
<Frame caption="Step 3: invite teammates by email and assign each a role, with the billing summary.">
  <img src="/images/workspaces-and-teams/workspace-creation-3.jpeg" alt="Workspace creation step 3 with rows of email and role inputs, an Add another button, and the seats billing summary" />
</Frame>

## Step 4: Create

Click **Create workspace**. The card on file is charged immediately for the first month's Team seats.
...
- **Adjust seats.** Adding seats is one click in **Settings → Billing**.
...
- [How billing works](/billing/plans), plans, seats, and metered usage.
- [Compute pricing](/billing/compute), CPU and memory rates.
